...
**P.S.** The console stores its fonts in `dev_rebug/data/font` if Rebug Toolbox with FileZilla is used. The font can be changed in **Settings: Theme Settings: Font**. If the tool detects that the firmware version is older than 4.81 or is using a non-PS3 user agent (e.g. Microsoft Xbox or Netscape Win16), all options are disabled.
